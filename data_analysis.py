...
def move_result_to_folder(fname, sas_ratio=0.033, sas_bounce_ratio=0.01):
    folder_str = 'lab_result/lab20170518/'
    sas_ratio_str = str(sas_ratio).translate(None,'.')
    sas_bounce_ratio_str = str(sas_bounce_ratio).translate(None,'.')
    folder_str = folder_str+sas_ratio_str+"-"+sas_bounce_ratio_str+'/'
    print folder_str
...
if __name__ == "__main__":
    ...
    iPhone_Type = raw_input("""input iPhone Type: 
        0: iPhone3GS
        1: iPhone4
        2: iPhone4s
        3: iPhone5
    ...
    mode_input = raw_input("""input analysis mode: 
        1: Seven Days Sale Moving Mean
        2: Total Sale\n""")

    mode = int(mode_input)
    
    sas_ratio_input = raw_input('Input Supply And Sale Ratio Threshold (0.033 or 0.05): ')
    sas_ratio = float(sas_ratio_input)
    sas_bounce_ratio_input = raw_input('Input Supply And Sale Bounce Ratio Threshold (0.01 or 0.02): ')
    ...
    os.system(move_result_to_folder(save_fname, SAS_RATIO_THRESH, SAS_BOUNCE_RATIO_THRESH))
